...
//					)
	...
	public Object arroundLog(ProceedingJoinPoint jp) {
		...
		*/
		...
		//System.out.println( "실행클래스:" + jp.getTarget() );
		//System.out.println( "실행메서드:" + jp.getSignature().toString() );
		//System.out.println( "실행메서드의 매개값" + Arrays.toString(jp.getArgs()) );
		
		long start = System.currentTimeMillis();
		
		log.info("실행클래스:" + jp.getTarget());
		log.info("실행메서드:" + jp.getSignature().toString());
		log.info("실행메서드의 매개값" + Arrays.toString(jp.getArgs()));
		...
		try {
			...
			//before
			result = jp.proceed(); //타겟메서드의 실행 - proceed 기준으로 after, before, afterThrowing(오류나면) 나뉨
			//after
			
			long end = System.currentTimeMillis();
			log.info("실행시간:" + (end-start) * 0.001);
			
			String path = "C:\\Users\\USER\\Desktop\\course\\spring\\spring_log\\24.08.06_log.txt";
